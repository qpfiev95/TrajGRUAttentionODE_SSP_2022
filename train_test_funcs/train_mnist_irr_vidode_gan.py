...
from model_structure.gan import *
...
def train_valid(train_iterator, valid_iterator, encoder_forecaster, optimizer, criterion):
    ...
    ### Discriminator#######################################
    netD_img, netD_seq, optimizer_netD = create_netD(
        irregular=True, extrap=True, sample_size=20, lr=cfg.MODEL.LR)
    ###########################################################
    if cfg.MODEL.TRAINING_MODE == "continue":
        ...
        netD_img.load_state_dict(model_params['model_state_dict_D_img'])
        netD_seq.load_state_dict(model_params['model_state_dict_D_seq'])
        optimizer_netD.load_state_dict(model_params['optimizer_state_dict_D'])
    ...
    while iter_id < cfg.MOVINGMNIST.TRAIN.MAX_ITER:
        if (iter_id + 1) % (cfg.MOVINGMNIST.TRAIN.UPDATE_ITER_INTERVAL) == 0:
        ...
        timestep_index += 1
        ###
        last_frame_train_data = train_data[-1]
        train_data = torch.permute(train_data, (1,0,2,3,4)) # BxSxCxHxW
        train_target_diff = torch.cat([last_frame_train_data.unsqueeze(0), train_target], dim=0)
        ## 2) Train model
        ...
        output, output_diff = encoder_forecaster(train_fo_timestep, train_data, train_en_timestep)
        ## 5) Compute loss function
        loss_G = criterion(output, train_target, output_diff, train_target_diff)
        ###
        ## loss_netD
        real = torch.permute(train_target, (1,0,2,3,4))
        fake = torch.permute(output, (1,0,2,3,4))
        loss_D = cfg.VidODE.LAMBDA_ADV * netD_seq.netD_adv_loss(real, fake, train_data)
        loss_D += cfg.VidODE.LAMBDA_ADV * netD_img.netD_adv_loss(real, fake, None)
        loss_adv_G = cfg.VidODE.LAMBDA_ADV * netD_seq.netG_adv_loss(fake, train_data)
        loss_adv_G += cfg.VidODE.LAMBDA_ADV * netD_img.netG_adv_loss(fake, None)
        loss_G += loss_adv_G
        ###
        ...
        if (iter_id + 1) % cfg.MOVINGMNIST.TRAIN.PRINT_ITER_INTERVAL == 0:
            common_loss = mse + mae + 0.05*ssim
            print("[{}] Iter: {}, Loss: {}, common loss: {}".format(time.ctime()[11:19], iter_id, loss_G, common_loss))
        ...
        if (iter_id+1) % (cfg.MOVINGMNIST.TRAIN.TEST_ITER_INTERVAL) == 0:  ### 2000
            ...
            evaluater.clear_all()
            train_data = torch.permute(train_data, (1, 0, 2, 3, 4))
            if cfg.MODEL.IRR_MODE == "forecaster":
        ...
        if (iter_id + 1) % (cfg.MOVINGMNIST.TRAIN.VALID_ITER_INTERVAL) == 0:  ### 8000 The duration of one epoch
            ...
            for i in range(cfg.MOVINGMNIST.VALID.VALID_NUM):
                with torch.no_grad():
                    ...
                    last_frame_valid_data = valid_data[-1]
                    valid_data = torch.permute(valid_data, (1, 0, 2, 3, 4))
                    valid_target_diff = torch.cat([last_frame_valid_data.unsqueeze(0), valid_target], dim=0)
                    encoder_forecaster.eval()
                    valid_output, valid_output_diff = encoder_forecaster(valid_fo_timestep, valid_data, valid_en_timestep)
                    loss_v = criterion(valid_output, valid_target, valid_output_diff, valid_target_diff)
                    valid_loss += loss_v.item()
                    valid_output_numpy = valid_output.detach().cpu().numpy()
                ###
                evaluater.update(train_target_numpy, output_numpy)
                mse, mae, ssim = evaluater.calculate_stat()
                v_avg_mse += mse
            ...
            common_valid_loss = (v_avg_mse + v_avg_mae + v_avg_ssim*0.05) / cfg.MOVINGMNIST.VALID.VALID_NUM
            with open(os.path.join(cfg.MODEL.TRAIN_IMAGE_PATH, 'result.txt'), 'a') as f:
                f.writelines("Validation task: Iter {}: ; Valid_loss: {}; Common_Valid_loss: {}\n".format(
                    iter_id, str(valid_loss), str(common_valid_loss)))
                f.close()
        ...
        if (iter_id+1) % (cfg.MOVINGMNIST.TRAIN.SAVE_ITER) == 0:  ### 8000
            ...
            torch.save({
                ...
                'model_state_dict_D_img': netD_img.state_dict(),
                'model_state_dict_D_seq': netD_seq.state_dict(),
                'optimizer_state_dict_D': optimizer_netD.state_dict(),
                'loss_G': loss_G,
            ...
            with open(os.path.join(cfg.MODEL.TRAIN_IMAGE_PATH, 'result.txt'), 'a') as f:
                f.writelines("Evaluation metrics during training process: AVG_MSE: {}; AVG_MAE: {}; AVG_SSIM: {}\n".format(
